...
currentVer=$(grep -Po '(?<="version": ")\d+\.\d+\.\d+' "$appParentDir"/$versFile 2>/dev/null)
latestVer=$(wget -qO - 'https://github.com/microsoft/vscode/releases/latest' | grep -Po '(?<=tag/)\d+\.\d+\.\d+' | head -1)
